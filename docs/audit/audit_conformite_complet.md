# 🛡️ Audit de Conformité Réglementaire — Projet SOC Wazuh
...
## ⚠️ Précision importante : SOC 1 vs SOC 2

**Vous avez mentionné « SOC 1 »**, mais attention :
- **SOC 1 (SSAE 18)** concerne les contrôles sur les **rapports financiers** (comptabilité). Il est **sans rapport** avec un projet SIEM/SOC.
- **SOC 2 Type II** concerne les contrôles de **sécurité, disponibilité, confidentialité** — c'est celui qui s'applique à votre projet.

> [!TIP]
> En soutenance, si vous citez « SOC », dites **SOC 2** et non SOC 1. Un recruteur ou un jury vous corrigera immédiatement si vous confondez les deux.
...
## 🏆 Points forts du projet (forces transversales)

Ces éléments sont remarquablement couverts et constituent vos **arguments massue** en soutenance :
...
|-------------|----------|:------:|--------|
| 21§2(a) | Politiques d'analyse des risques et de sécurité SI | ✅ | EBIOS RM, PSSI, IAM, charte |
| 21§2(b) | Gestion des incidents | ✅ | Plan IR 7 étapes, PCA/PRA |
| 21§2(c) | Continuité, sauvegardes, reprise | ✅ | PCA/PRA, politique de sauvegarde 3-2-1 |
| 21§2(d) | Sécurité de la chaîne d'approvisionnement | 🟡 | Vérification SHA-256 du MSI mais pas de gestion supply chain globale |
| 21§2(e) | Sécurité dans l'acquisition et développement, gestion des vulnérabilités | 🟡 | Ansible IaC, SCA mais **pas de patch management formel** |
| 21§2(f) | Évaluation de l'efficacité des mesures | ✅ | 20 KPIs, calendrier de tests PCA/PRA, playbooks Red Team |
| 21§2(g) | Pratiques de cyberhygiène et formation | 🟡 | CIS benchmarks via SCA ; formation mentionnée mais non prouvée |
| 21§2(h) | Cryptographie et chiffrement | ✅ | TLS 1.2+, DPAPI AES-256, Ansible Vault, WireGuard |
| 21§2(i) | Sécurité RH, contrôle d'accès, gestion des actifs | ✅ | Politique IAM, RBAC, cycle de vie comptes, inventaire agents |
| 21§2(j) | Authentification multi-facteur, communications sécurisées | 🟡 | MFA pour Tailscale/Proxmox mais **pas pour le Dashboard Wazuh** |

#### Article 23 — Obligations de signalement

| Sous-article | Exigence | Statut |
|-------------|----------|:------:|
| 23§4(a) | Alerte précoce sous 24h | 🟡 (timings définis mais mappés CNIL, pas CSIRT NIS2) |
| 23§4(b) | Notification d'incident sous 72h | 🟡 (72h mentionné pour la CNIL, adaptable) |
| 23§4(c) | Rapport intermédiaire | ❌ |
| 23§4(d) | Rapport final sous 1 mois | ❌ |
...
|---------|----------|:------:|
| **CC3** (Évaluation des risques) | Analyse de risques, traitement | ✅ EBIOS RM |
| **CC4** (Activités de surveillance) | Monitoring continu | ✅ Wazuh SIEM, 20 KPIs |
| **CC5** (Activités de contrôle) | Gestion des changements | ✅ ITIL v4, Git |
| **CC6.1** (Accès logique et physique) | Contrôles d'accès | 🟡 Logique ✅ / Physique ❌ |
| **CC6.2-6.6** (Identité, RBAC) | Gestion des identités | ✅ IAM, RBAC, cycle de vie |
| **CC7.1-7.4** (Détection, réponse) | Monitoring, incidents | ✅ Règles custom, IR plan |
| **CC8.1** (Gestion des changements) | Changements autorisés | ✅ RFC, CAB, PIR |
| **CC9.2** (Risques fournisseurs) | Gestion tiers | ❌ Aucune politique |
| **A1** (Disponibilité) | Infrastructure, reprise | ✅ PCA/PRA, Ansible |
| **C1** (Confidentialité) | Classification, suppression | 🟡 Registre RGPD mais pas de classification formelle |
...
✅ **Solide** : Art. 5.1 (principes), Art. 6 (base légale = intérêt légitime), Art. 24-25 (responsabilité, privacy by design), Art. 30 (registre des traitements), Art. 32 (sécurité), Art. 33 (notification CNIL <72h)

🟡 **Partiels** : Art. 12-14 (information des personnes), Art. 15-22 (droits des personnes), Art. 28 (sous-traitant — Tailscale Inc.), Art. 35 (PIA/DPIA)
...
|----------|:------:|-------------|
| **IDENTIFY** | ✅ | EBIOS RM, inventaire des actifs |
| **PROTECT** | ✅ | IAM, chiffrement, SCA, sauvegardes |
| **DETECT** | ✅ | Wazuh SIEM, règles custom, MITRE ATT&CK |
| **RESPOND** | ✅ | Plan IR 7 étapes, matrice de communication |
| **RECOVER** | ✅ | 5 PRAs détaillés, tests planifiés |
| **GOVERN** | 🟡 | Politiques existantes mais gouvernance légère |

---

## 🔴 Top 5 des lacunes critiques (transversales)
...
> [!NOTE]
> Ces lacunes sont **normales et attendues** pour un projet académique déployé sur un périmètre de staging. Un SOC de production en entreprise nécessiterait 12 à 18 mois supplémentaires pour atteindre une couverture ISO 27001 complète. Le travail réalisé ici couvre les piliers fondamentaux (détection, accès, continuité, risques) de manière remarquable pour un projet de fin d'études.

---

## ✅ Verdict Final

> **Votre projet ne peut pas prétendre être « conforme ISO 27001 » ou « conforme NIS 2 »** — la conformité implique un audit formel par un organisme accrédité. En revanche, votre projet **s'aligne fortement** sur les exigences fondamentales de ces référentiels, et le travail de documentation GRC est **exceptionnellement complet pour un projet académique**.

**Formulation recommandée en soutenance :**

> *« Ce projet s'inscrit dans une démarche d'alignement avec les référentiels ISO 27001, NIS 2, NIST CSF et les recommandations de l'ANSSI. Sans prétendre à une certification formelle, il implémente les contrôles fondamentaux de détection, de gestion des accès, de continuité d'activité et de gouvernance des risques exigés par ces standards. »*
