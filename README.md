...
│   ├── demandes/
│   │   ├── fiche_demande_active_directory.md # Fiche de demande de droits AD
│   │   ├── fiche_demande_proxfibre.md        # Fiche réseau/infra ProxFibre
│   │   └── mail_tuteur_stage.txt       # Modèle d'email d'accompagnement
│   ├── audit/
│   │   ├── audit_conformite_referentiels.md  # Alignement ANSSI, NIST, ISO 27001
│   │   └── attack_playbooks_and_detection_matrix.md # Scénarios d'attaque et matrice de détection
│   └── guides/
│       ├── gpo_deployment_guide.md   # Guide technique complet pour l'admin AD
│       └── document_suivi_projet_v2.md  # Journal d'avancement du projet
...
2. Générez le secret d'enrôlement chiffré sur une machine de test en contexte local `SYSTEM` (ou via le guide dans `docs/guides/gpo_deployment_guide.md`) avec le script `Initialize-WazuhDeployCredential.ps1`.
...
* **Conformité réglementaire :** Mappage complet des contrôles du SOC sur le guide d'hygiène informatique de l'ANSSI et les contrôles CIS v8 disponible dans `docs/audit/audit_conformite_referentiels.md`.
